	...
	if(layer_tilemap_exists(layer_id, map_id)){
		for(var i = 0; i<abs(bbox_left-bbox_right)/16;i++)
			for(var j = 0; j<abs(bbox_top-bbox_bottom)/16; j++){
				...
				part_particles_create(global.part_system_front,x+xMult+8,y+yMult+8,obj_partManager.partSmokeBlastSmall,1);
				for(var k = 0; k<random_range(1,2); k++){
					instance_create_depth(x+xMult+8,y+yMult+8,depth,obj_debris);
				}
			}
	}
}